...
class Dataset(object):
    ...
    def saveDataAsTable(self, tbl_name, data, if_exists=None, index_label=None):
        with self.connect() as conn:
            try:
                data.to_sql(tbl_name, con=conn, if_exists=if_exists, index_label=index_label)
...
class StockData(object):
    ...
    def getStockDataViaTushare(self, symbol, start=None, end=None):
        if start:start=pd.to_datetime(start).strftime("%Y-%m-%d")
        if end:end=pd.to_datetime(start).strftime("%Y-%m-%d")
        if not start: start='1900-01-01'
    ...
    def getLocalStockData(self, symbol, start=None, end=None):
        ...
        if end:end=pd.to_datetime(start).strftime("%Y-%m-%d")
        if not start: start='1900-01-01'
        sql=f'select date, open, close, high, low, volume from {tbl_name} where date > "{start}"'
    ...
    def updateStock(self, symbol, data):
        try:
            self.dataset.saveDataAsTable(self.getTableName(symbol), data=data, if_exists='append')
        except sqlalchemy.exc.OperationalError:
    ...
    def updateStockInfo(self):
        ...
        self.dataset.saveDataAsTable(self.tbl_name_symbols, info,if_exists='replace', index_label='symbol')
        return info        
